...
这个 shell 说和听都支持 **英语、俄语、西班牙语和中文**。分流前缀、撤销、记忆、
后台任务、Telegram 通道、权限提问的是与否，四种语言里都有；它说回来的每一句
也一样。手机界面同样翻成了这四种。

说和听是故意分开决定的：

- **它听什么**根本不需要配置。所有语言的命令短语同时匹配——shell 事先无从知道
  你下一句会用哪种语言说，而要求你开口前先拨个开关，等于取消了免手操作的意义。
  这样做是安全的，因为这些是口令而不是文章：表都很短，而且是挑着不互相撞车的。
- **它说什么**是每句话单独决定的：先看手机连接时声明了什么，再看你刚才实际用的
  是哪种语言，然后是 `VOICE_LANG`，最后才是英语。聊到一半改说英语，回答就会用
  英语回来，不用去翻设置。

这些表在 [`daemon/voice_claude/lexicon.py`](daemon/voice_claude/lexicon.py)（听）
和 [`daemon/voice_claude/i18n.py`](daemon/voice_claude/i18n.py)（说）里，都是普通
的字典。加第五种语言就是加一列，漏了什么 `tests/test_i18n.py` 会告诉你。

如实说明限制：俄语的措辞是每天在用的那一版，英语是参照译文。西班牙语和中文翻得
用心，但没有母语者审校过，尤其是命令短语，很希望真正在用的人来说哪些听着别扭。
希伯来语只作为识别语言存在——shell 没有它的短语表，所以用希伯来语说话时，这东西
就是一条通往 Claude 的语音管道，再没别的。
