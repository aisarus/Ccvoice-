...
class Prefs(context: Context) {
    ...
    /**
     * Не стирать прошлую ошибку при запуске.
     *
     * Служба перезапускается сама через секунды после падения, и очистка в
     * `onCreate` означала, что о ночном падении человек не узнает никогда:
     * экран пуст, кнопка «скопировать ошибку» копирует пустоту. Чистит её
     * тот, кто её прочитал.
     */
    fun keepPreviousError() = Unit
