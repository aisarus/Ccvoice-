...
Приложение и свой сервер — в [README](../README.ru.md).
...
| Подписка Claude (Pro или Max) | [claude.ai](https://claude.ai) | ей авторизуется Claude Code; см. оговорку под таблицей |
...
> **Про подписку.** Подписка Claude (Pro или Max) годится для инстанса, который
> ты держишь для себя. С февраля 2026 Anthropic не разрешает использовать
> OAuth-доступ подписки из сторонних продуктов, поэтому сервису на Render, с
> которым разговаривают другие люди, нужен свой `ANTHROPIC_API_KEY`. Публичный
> адрес плюс общий токен — это уже сервис: либо адрес никому не давать, либо
> ключ.
